...
class RepositoryJobsServiceSpec extends WordSpec with Matchers with ScalaFutures with MockitoSugar {
  ...
  "Update" should {
    "fetch all builds from ci-open and ci-dev and persist them" in {
      ...
      val builds = List(validBuildDev, validBuildOpen)

      val expectedResult = UpdateResult(nSuccesses = 2, nFailures = 0)
